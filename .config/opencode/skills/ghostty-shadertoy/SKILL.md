---
name: ghostty-shadertoy
description: Create and edit custom shaders for Ghostty terminal using Shadertoy-compatible GLSL ES 3.0
---
...
### Blending with Terminal Content

Shaders that fully obscure the terminal are useless. Always blend with `iChannel0`:
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Your shader effect
    vec3 effectColor = someEffect(uv);

    // Blend based on terminal luminance (dark areas = more effect)
    float luminance = dot(terminalColor.rgb, vec3(0.299, 0.587, 0.114));
    float mixFactor = smoothstep(0.0, 0.3, 1.0 - luminance);

    // 0.25 = subtle, 0.5 = moderate, 0.75 = dominant
    fragColor = vec4(mix(terminalColor.rgb, effectColor, mixFactor * 0.25), terminalColor.a);
}
```

### Luminance-Based Masking

Use terminal content brightness to control where effects appear:

```glsl
// Effect only on dark backgrounds (behind text)
float mask = smoothstep(0.3, 0.0, luminance);

// Effect only on bright areas (on text)
float mask = smoothstep(0.0, 0.3, luminance);
```

## UV Orientation

Shadertoy uses bottom-left origin. Ghostty does too, but some effects expect top-left:

```glsl
// Flip Y for effects designed with top-left origin
uvGen.y = 1.0 - uvGen.y;
```

## Converting Shadertoy Texture Dependencies

When a Shadertoy shader uses `iChannel0` for noise/texture input (not the terminal):

```glsl
// ORIGINAL (Shadertoy) - uses iChannel0 as blue noise texture
float noise(vec2 x) {
    vec2 p = floor(x);
    float a = texture(iChannel0, p / 1024.0).x;
    // ...
}

// ADAPTED (Ghostty) - generate noise procedurally
float hash(vec2 p) {
...
Fractal Brownian Motion is expensive. Optimize by reducing octaves:

```glsl
// EXPENSIVE: 8 octaves
float fbm(vec2 x) {
    float a = 0.0, b = 0.5;
    for (int i = 0; i < 8; i++) {  // 8 texture lookups per pixel
        a += b * noise(x);
        x *= 2.0;
        b *= 0.5;
    }
...
// CHEAP: 5 octaves (usually sufficient)
float fbm(vec2 x) {
    float a = 0.0, b = 0.5;
    for (int i = 0; i < 5; i++) {  // 40% fewer lookups
        a += b * noise(x);
        x *= 2.0;
        b *= 0.5;
    }
...
Also consider cheaper interpolation:

```sl
// QUINTIC (original Shadertoy) - expensive
vec2 u = f*f*f*(f*(f*6.0-15.0)+10.0);

// CUBIC (cheaper) - barely noticeable difference
